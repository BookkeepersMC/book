...
public class DownloadTargetMappingJarTask extends DefaultMappingsTask {
    ...
    public DownloadTargetMappingJarTask() {
        ...
        targetMappingsFile.convention(() -> getProject().file(TARGET_MAPPINGS + "/book-mappings-" + checkExists.getTargetVersion().orElse(Constants.MAPPINGS_VERSION) + "/mappings/mappings.tiny"));
        targetUnpickFile.convention(() -> getProject().file(TARGET_MAPPINGS + "/book-mappings-" + checkExists.getTargetVersion().orElse(Constants.MAPPINGS_VERSION) + "/extras/unpick.json"));
        targetUnpickDefinitionsFile.convention(() -> getProject().file(TARGET_MAPPINGS + "/book-mappings-" + checkExists.getTargetVersion().orElse(Constants.MAPPINGS_VERSION) + "/extras/definitions.unpick"));
        targetUnpickRemappedDefinitionsFile.convention(() -> getProject().file(TARGET_MAPPINGS + "/book-mappings-" + checkExists.getTargetVersion().orElse(Constants.MAPPINGS_VERSION) + "remapped-unpick.unpick"));
        targetUnpickConstantsFile.convention(() -> getProject().file(TARGET_MAPPINGS + "/book-mappings-" + checkExists.getTargetVersion().orElse(Constants.MAPPINGS_VERSION) + "-constants.jar"));
    }
    ...
    public void downloadTargetMappings() throws IOException {
        ...
        File targetMappingsJar = getProject().file(TARGET_MAPPINGS + "/book-mappings-" + targetVersion + "-v2.jar");
        this.startDownload()
                .src("https://bookkeepersmc.github.io/m2/com/bookkeepersmc/book-mappings/" + targetVersion + "/book-mappings-" + targetVersion + "-v2.jar")
                .dest(targetMappingsJar)
                .download();

        this.startDownload()
                .src("https://bookkeepersmc.github.io/m2/com/bookkeepersmc/book-mappings/" + targetVersion + "/book-mappings-" + targetVersion + "-constants.jar")
        ...
        getProject()
                ...
                .getAsFileTree()
                .visit(fileVisitDetails -> fileVisitDetails.copyTo(getProject().file(TARGET_MAPPINGS + "/book-mappings-" + targetVersion + "/" + fileVisitDetails.getRelativePath())));
    }
