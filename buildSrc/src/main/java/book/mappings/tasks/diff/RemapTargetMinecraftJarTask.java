...
public class RemapTargetMinecraftJarTask extends MapJarTask {
    ...
    public RemapTargetMinecraftJarTask() {
        ...
        inputJar.convention(() -> getProject().file(DownloadTargetMappingJarTask.TARGET_MAPPINGS + "/book-mappings-" + checkExists.getTargetVersion().orElse(Constants.MAPPINGS_VERSION) + "-unpicked.jar"));
        mappingsFile.set(downloadTarget.getTargetMappingsFile());
        outputJar.convention(() -> getProject().file(DownloadTargetMappingJarTask.TARGET_MAPPINGS + "/book-mappings-" + checkExists.getTargetVersion().orElse(Constants.MAPPINGS_VERSION) + "-named.jar"));
    }
