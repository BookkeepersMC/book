...
public abstract class TinyJarTask extends Jar implements MappingsTask {
    ...
    public TinyJarTask() {
        ...
        this.getMappings().convention(this.getTaskNamed(MergeTinyTask.TASK_NAME, MergeTinyTask.class).getOutputMappings());

        this.getArchiveFileName().set(String.format("%s-%s.jar", Constants.MAPPINGS_NAME, Constants.MAPPINGS_VERSION));
        this.getDestinationDirectory().set(getProject().file("build/libs"));
