...
async function main() {
    ...
    try {
        ...
        if (diff === "") {
            ...
            await commentDiff(`With commit ${context.sha}, ${modified_files} file(s) were updated with ${added_lines} line(s) added and ${removed_lines} removed compared to the latest book version.`, diff, comment);
        }
    } catch (err) {
        console.error(err);
